...
#predictions
predR <- ggemmeans(fitR, terms = c("time [all]", "stimuli"), type = "fe", ci.lvl = 1-alphaci)
...
#predictions
predNR <- ggemmeans(fitNR, terms = c("time [all]", "stimuli"), type = "fe", ci.lvl = 1-alphaci )
...
##OR of the last session
#get data
datosR$session <- as.numeric(datosR$session)
lastSes <- sapply(IDs, function(x) max(datosR$session[datosR$Rat == x]))
dataLast <- do.call(rbind, lapply(IDs, function(x) subset(datosR[datosR$Rat == x,], session == lastSes[x])))
dataLast$stimuli <- as.factor(dataLast$stimuli)

fitRLast <- glmer(rate ~ stimuli + (1|ID), data = dataLast, family = binomial("logit"), weights = offset)
sumLast <- summary(fitRLast)

#OR
1/exp(sumLast$coefficients[2,1])

#p for dynamic
pD<-exp(sumLast$coefficients[1,1])
#p for static
pS<-exp(sumLast$coefficients[1,1] + sumLast$coefficients[2,1])

#%
(pD-pS)*100 / pS

#p-values
pR <- emmeans(fitRLast, specs = "stimuli")
summary(contrast(pR, type = "response"))
...
HR_NRS <- ggemmeans(survFit_NRS, terms = c("NormSes [all]"), type = "fe", ci.lvl = 1-alphaci )
...
HR_NRD <- ggemmeans(survFit_NRD, terms = c("NormSes [all]"), type = "fe", ci.lvl = 1-alphaci )
...
HR_RS <- ggemmeans(survFit_RS, terms = c("NormSes [all]"), type = "fe", ci.lvl = 1-alphaci )
...
HR_RD <- ggemmeans(survFit_RD, terms = c("NormSes [all]"), type = "fe", ci.lvl = 1-alphaci )

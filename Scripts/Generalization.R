...
#plot
surv.eff.N <- ggpredict(surv.mod.N, terms = c("stimuli"), type = "surv", ci.lvl = 1-alphaci)
...
surv.eff.N$group <- factor(surv.eff.L$group, 
                           levels = c("Generalize_Reward", "Generalize_NonRew", "Familiar", "Novelty_Control"))
...
time.effects.gea <- ggpredict(fitTime.G, terms = c("time[all]", "stimuli"), type = "fe")

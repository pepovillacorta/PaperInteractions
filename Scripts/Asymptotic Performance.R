...
#subset D setup
dataLast <- dataLast[dataLast$setup == "D",]
...
pred.Lea <- ggemmeans(fit.Lea, terms = c("stimuli"), type = "fe", ci.lvl = 1-alphaci)
...
### PLOT Survival ----
surv.eff.L <- ggpredict(surv.mod.L, terms = c("stimuli"), type = "surv", ci.lvl = 1-alphaci)
...
rec_names <- unique(tgdata$ID)

for(j in 1:length(rec_names)){
  for(i in 1:length(SceneNames_5)){
    for(h in 1:max(tgdata$stID.within[tgdata$stimuli == SceneNames_5[i] & tgdata$ID == rec_names[j]]))
      tgdata$each[tgdata$stimuli == SceneNames_5[i] & tgdata$ID == rec_names[j] & tgdata$stID.within == h] <- rep(
        seq(1,length(tgdata$each[tgdata$stimuli == SceneNames_5[i] & tgdata$ID == rec_names[j] & tgdata$stID.within == h] ) / bin.g ), each = bin.g)
  }
}
...
time.effects.lea <- ggpredict(fitTime.G, terms = c("time[all]", "stimuli"), type = "fe")
